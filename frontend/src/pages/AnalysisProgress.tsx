...
export default function AnalysisProgress() {
  ...
  const currentStepKey = activeStep > 0 ? STEPS[Math.min(activeStep - 1, STEPS.length - 1)] : null;
  const progressAnnouncement = done
    ? t("progress_step8", lang)
    : `${currentStepKey ? t(currentStepKey, lang) : t("progress_title", lang)} · ${pct}%`;
  ...
  return (
    <div className="min-h-full flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-[480px] anim-fade-up">
        ...
        {/* Idea snippet */}
        <div className="rounded-xl px-4 py-3.5 mb-8" style={{ background: "var(--surface)", border: "1px solid var(--border-c)" }} data-reveal>
        ...
        {!error && (
          <section aria-live="polite" aria-busy={!done} aria-label={t("progress_title", lang)} data-reveal>
            <p className="sr-only">{progressAnnouncement}</p>
