...
async function readJsonBody(request) {
  ...
  for await (const chunk of request) {
    ...
    if (size > MAX_BODY_BYTES) {
      const error = new ValidationError('request body is too large', 'request');
  ...
  try {
    ...
    throw new ValidationError('must contain valid JSON', 'request');
  }
}

function statusForError(error) {
  if (error instanceof ValidationError) return 400;
  if (error?.name === 'ServiceConfigurationError') return 503;
  return 502;
}

function codeForError(error) {
  if (error instanceof ValidationError) return 'INVALID_REQUEST';
  if (error?.name === 'ServiceConfigurationError') return 'SERVICE_NOT_CONFIGURED';
  ...
  return async function requestHandler(request, response) {
    ...
    if (url.pathname === '/api/analyze' && request.method !== 'POST') {
      writeJson(
        response,
        405,
        { error: { code: 'METHOD_NOT_ALLOWED', message: 'Use POST for this endpoint.', requestId } },
        { allow: 'POST' },
      );
      return;
    }
    ...
    if (request.method === 'POST' && url.pathname === '/api/analyze') {
      ...
      try {
        ...
        const report = await analyzeIdea(body, services);
        ...
        const message =
          status === 400
            ? error.message
            : status === 503
              ? 'Live services are not configured. Review the server environment.'
    ...
    if (publicDir !== null && (await serveStatic(request, response, publicDir, url.pathname))) {
      return;
    }
